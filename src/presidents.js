...
function getNames(presidentsArr) {
  return presidentsArr.map((presidentname) => presidentname.name);
}

// Iteration 2 | Democratic Presidents - `filter()`
function getDemocraticPresidents(presidentsArr) {
  return presidentsArr.filter((filtered) => {
    if (filtered.party === "Democratic") {
...
function countYearsInOffice(presidentsArr) {
  return presidentsArr.reduce((total, presidentIn) => {
    if(!presidentIn.leftOffice) return total
   
    return total + (presidentIn.leftOffice - presidentIn.tookOffice);
